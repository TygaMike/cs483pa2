...
class RSA(object):
    ...
    def encrypt(self, number_to_encrypt):
        ...
        pad_num = self.pckspad(encrypted_number)
        self.write_to_file(pad_num)

    def decrypt(self, number_to_decrypt):
        return pow(number_to_decrypt, self.e_or_d, self.N)

    def pckspad(self, upk):
        r = int(self.n) / 2
        pad = random.getrandbits(r)
        padkey = str(0x00) + str(0x02) + str(pad) + str(0x00) + str(upk)
        return padkey
